...
void solve()
{
    ...
    // Map to store minimum value for each diagonal (key = i - j)
    map<int, int> diagMin;

    // Traverse the grid and compute minimum for each diagonal
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            int diag = i - j; // Diagonal identifier
            if (diagMin.find(diag) == diagMin.end() && v[i][j] < 0) {
                diagMin[diag] = v[i][j]; // Initialize with the first value
            } else {
                diagMin[diag] = min(diagMin[diag], v[i][j]); // Update minimum
            }
        }
    }

    // Sum the absolute values of the minimum elements from all diagonals
    int ans = 0;
    for (auto it : diagMin) {
        ans += abs(it.second);
       // print("Diagonal " + to_string(it.first) + ": " + to_string(it.second));
    }
    print(ans);
}
