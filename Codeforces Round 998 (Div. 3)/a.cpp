...
void solve()
{
    ...
    // Debug: Print sorted cards
    //cout << "Sorted cards for each cow:" << endl;
    for (int i = 0; i < n; i++) {
       // cout << "Cow " << i << ": ";
       // for (int card : v[i]) cout << card << " ";
        //cout << endl;
    }

    // Find minimum cards for each cow
    vector<int> mins(n);
    for (int i = 0; i < n; i++) mins[i] = v[i][0];

    // Determine turn order
    vector<int> ord(n);
    iota(ord.begin(), ord.end(), 0);
    stable_sort(ord.begin(), ord.end(), [&](int a, int b) { return mins[a] < mins[b]; });

    // Debug: Print turn order
    // cout << "Turn order (0-based): ";
    ...
    for (int i : ord) {
        if (v[i][0] <= prev) {
            //cout << "Validation failed at cow " << i << endl;
            cout << -1 << endl;
            return;
        }
        prev = v[i][0];
